# 👋 Hi, I'm Willien

🎓 Computer Engineering Student at ESIEE Paris  
🧠 Passionate about software development, open source, and innovation  
🔍 Currently seeking an apprenticeship at Thales in Open & Inner Source development

---

## 🚀 Highlighted Projects

- [Donation App](https://github.com/Kuyakii/Donation-App-mobile) — Mobile app for donations with QR codes, Stripe integration, map, and authentication

---

## 🛠️ Tech Stack

```bash
Languages: Python, Java, JavaScript, PHP, SQL, Kotlin, HTML/CSS
Frameworks: React, Node.js, Bootstrap, Pygame
Tools: Git, GitHub, GitLab, Android Studio, VSCode, XAMPP, MariaDB, Trello, IntelliJ IDEA, WebStorm, PhpStorm, Excel, Oracle SQL
```

---

## 📫 Contact

- [LinkedIn](https://www.linkedin.com/in/willien-hu-7607342a6/)
- Email: huwillien56@gmail.com
---

> *I’m always curious to learn new things and contribute to impactful projects. Feel free to explore my repositories or reach out if you'd like to connect!*
